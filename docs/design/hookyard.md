...
| Codex | Unconfirmed. On `turn_end` (native `Stop`) a deny renders top-level `{"decision":"block","reason":"..."}`, and nothing once `stop_hook_active` is true — §11.3 | **Probed — a channel exists, not yet wired here** | `SessionStart` and `UserPromptSubmit` both deliver `additionalContext`, proven at wire level: the marker arrives as a `developer`-role model input Codex labels `hooks.additional_context` (`fixtures/codex-advisory/outcome-0.154.0-positive.md`, codex-cli 0.154.0). The previous "no advisory channel at all — a settled boundary" rested on an absence of observation and was false. `HasAdvisorySlot` still returns false for Codex: a known gap, not an engine property — the slot waits on a Render arm. `PreToolUse`/`PostToolUse`/`SubagentStart` are documented but unprobed, and a 401'd turn cannot reach them. Separately, the deny-arm and default-timeout clause in this cell was stale for months: both were already fixture-verified (`hook-payloads/codex-pre_tool_use-DENY.json`, `codex-user_prompt_submit-TICK.json`). Issue #87 |
